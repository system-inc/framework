...
function relativeId(id) {
    if (typeof process === 'undefined' || !isAbsolute(id))
        return id;
    return sysPath.relative(process.cwd(), id);
}
...
function umd(magicString, { accessedGlobals, dependencies, exports, hasExports, indentString: t, intro, namedExportsMode, outro, varOrConst, warn }, { amd: { define: amdDefine, id: amdId }, compact, esModule, extend, externalLiveBindings, freeze, interop, name, globals, noConflict, strict }) {
    ...
    const amdParams = (amdId ? `'${amdId}',${_}` : ``) + (amdDeps.length ? `[${amdDeps.join(`,${_}`)}],${_}` : ``);
...
class IfStatement extends NodeBase {
    ...
    parseNode(esTreeNode) {
        ...
        this.consequent = new this.context.nodeConstructors[esTreeNode.consequent.type](esTreeNode.consequent, this, this.consequentScope);
        if (esTreeNode.alternate) {
            this.alternateScope = new TrackingScope(this.scope);
            this.alternate = new this.context.nodeConstructors[esTreeNode.alternate.type](esTreeNode.alternate, this, this.alternateScope);
        }
        super.parseNode(esTreeNode);
    }
    render(code, options) {
...
class Chunk$1 {
    ...
    generateIdPreserveModules(preserveModulesRelativeDir, options, existingNames, unsetOptions) {
        ...
        if (isAbsolute(id)) {
            ...
            path = relative(preserveModulesRelativeDir, `${sysPath.dirname(sanitizedId)}/${renderNamePattern(pattern, 'output.entryFileNames', {
    ...
    getDependenciesToBeDeconflicted(addNonNamespacesAndInteropHelpers, addInternalDependencies, addDependenciesWithoutBindings, interop) {
...
async function resolveId(source, importer, preserveSymlinks, pluginDriver, skip) {
    ...
    return addJsExtensionIfNecessary(sysPath.resolve(importer ? sysPath.dirname(importer) : sysPath.resolve(), source), preserveSymlinks);
}
    ...
    constructor(options, watcher) {
        ...
        this.getModuleInfo = (moduleId) => {
            ...
            return {
                ...
                id: foundModule.id,
                implicitlyLoadedAfterOneOf: foundModule instanceof Module ? Array.from(foundModule.implicitlyLoadedAfter, getId) : [],
                implicitlyLoadedBefore: foundModule instanceof Module ? Array.from(foundModule.implicitlyLoadedBefore, getId) : [],
                importedIds,
...
const getAcorn = Parser => {
  ...
  for (let cur = Parser; cur && cur !== acorn$1.Parser; cur = getPrototype(cur)) {
...
function numericSeparator(Parser) {
  const acorn$1 = Parser.acorn || acorn;
  const withAcornBigIntSupport = (acorn$1.version.startsWith("6.") && !(acorn$1.version.startsWith("6.0.") || acorn$1.version.startsWith("6.1."))) || acorn$1.version.startsWith("7.");

  return withAcornBigIntSupport ? withAcornBigInt(acorn$1, Parser) : withoutAcornBigInt(acorn$1, Parser)
}
