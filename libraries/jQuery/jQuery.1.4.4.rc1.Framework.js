/*!
 * jQuery JavaScript Library v1.4.4rc1
 * http://jquery.com/
 *
 ...
 *
 * Date: Mon Oct 25 13:25:10 2010 -0700
 */
...
jQuery.extend({
	queue: function( elem, type, data ) {
		//console.log('jQuery.queue', elem, type, data);
			...
			if ( id ) {
				...
				if ( data && data.events ) {
					for ( var type in data.events ) {
						if(data.events.hasOwnProperty(type)) {
							if ( special[ type ] ) {
...
jQuery.fn.extend({
	...
	animate: function( prop, speed, easing, callback ) {
		...
		//Console.out('Animate!', prop, optall);

		if ( jQuery.isEmptyObject( prop ) ) {
			//Console.out('giving up!');

			return this.each( optall.complete );
		}

		var fnToCall = optall.queue === false ? "each" : "queue";

		//Console.out('fntocall!', fnToCall);

		return this[ fnToCall ](function() {
			// this does not always have a nodeName when running the
			// test suite
