...
class Ffmpeg {
	...
	static async execute(input, argumentsArray) {
		...
		// If the input is an archived file system object
			...
			inputStream = await input.toReadStream();
		}
	...
	static async resizeImage(input, width, height) {
		...
		var ffmpegStandardOutStream = await Ffmpeg.execute(input, [
			// Scale
			'-vf',
			'scale='+width+':'+height,
	...
	static async imageFromVideoAtPosition(input, position, width, height) {
		// -1 will use automatically aspect ratio
		if(!width) {
			width = -1;
		}
